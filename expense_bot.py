...
# На сервере без графического окружения matplotlib обязан рисовать без дисплея.
matplotlib.use("Agg")
...
# Распознавание чеков через официальный API DeepSeek (OpenAI-совместимый).
# Ключ берётся из переменной окружения DEEPSEEK_API_KEY.
LLM_BASE_URL = "https://api.deepseek.com"
LLM_API_URL = f"{LLM_BASE_URL}/chat/completions"
LLM_API_KEY_ENV = "DEEPSEEK_API_KEY"
# Изображения принимает только deepseek-flash: у deepseek-v4-pro зрения нет.
# Список моделей и цены: https://api-docs.deepseek.com/quick_start/pricing
LLM_MODELS = [
    "deepseek-flash",
]
RECEIPT_PROMPT = (
...
def _llm_chat(content, api_key: str):
    ...
    for model in LLM_MODELS:
        payload = {
            ...
            # Режим размышления у DeepSeek включён по умолчанию и тратит отдельные
            # токены; для распознавания чека цепочка рассуждений не нужна.
...
# httpx пишет INFO на каждый запрос вместе с URL, а в URL Telegram лежит токен бота:
# без этого секрет оседает в journald на каждой итерации опроса. Оставляем только
# предупреждения и ошибки — реальные сбои связи всё равно будут видны.
...
# Пояс пользователей: Барнаул, UTC+7. В России перехода на летнее время нет,
# поэтому смещение круглый год постоянное.
LOCAL_TZ = ZoneInfo("Asia/Barnaul")


def local_now() -> datetime:
    """Текущее время по Барнаулу."""
    return datetime.now(LOCAL_TZ)


def local_today() -> date:
    """Сегодняшняя дата по Барнаулу — по ней считаются месяцы и недели."""
    return local_now().date()


def local_stamp() -> str:
    """Метка времени для базы: локальное время Барнаула БЕЗ смещения.

    Без смещения намеренно: SQLite-функция date() пересчитывает значения со
    смещением в UTC, и тогда фильтры «за день» и «за месяц» поехали бы на сутки.
    """
...
def add_custom_category(name: str) -> bool:
    ...
    try:
        ...
        # close() откатывает незавершённую транзакцию. Без этого отклонённый дубликат
        # оставлял открытое соединение с блокировкой, и следующая запись падала
        # с "database is locked" (соединение живёт до сборки мусора из-за цикла cursor↔conn).
        conn.close()
...
def add_goal(name: str, target_amount: float) -> int:
    ...
    try:
        ...
        cursor.execute(
            ...
            (name, target_amount, 0, local_stamp()),
        )
        ...
        # см. add_custom_category: закрываем соединение на обоих путях
        conn.close()
...
def add_expense(user_id: int, user_name: str, item: str, amount: float, category: str) -> int:
    ...
    cursor.execute(
        ...
        (user_id, user_name, item, amount, category, local_stamp()),
    )
    expense_id = cursor.lastrowid
    conn.commit()
    conn.close()
...
async def models_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Вытаскиваем id и отмечаем те, что принимают картинки. Каталог DeepSeek
    # небольшой, и изображения понимает только deepseek-flash.
    ids = [m.get("id", "") for m in models if isinstance(m, dict) and m.get("id")]
    vision_ids = [i for i in ids if i in LLM_MODELS]
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    month_name = local_today().strftime("%B %Y")
    report = f"📊 <b>Отчет за {month_name}</b>\n\n"
...
async def chart_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📝 Свой диапазон", callback_data="chart_custom_start")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def weekly_summary(context: ContextTypes.DEFAULT_TYPE) -> None:
    # Получаем текущую дату (по Барнаулу) и вычисляем начало и конец недели (пн-вс)
    today = local_today()
...
def main() -> None:
    ...
    # Настраиваем еженедельную сводку: задача идёт каждый день в 19:00 МСК,
    # а weekly_summary сама отправляет сводку только по воскресеньям.
    # В PTB нет run_weekly, а параметр days нумеруется в cron-нотации и НЕ совпадает
    # с date.weekday(): days=(6,) планировал субботу вместо воскресенья, и сводка
    # не уходила никогда. Поэтому расписание дневное, а день проверяет сама функция.
    # JobQueue существует только при установке extra job-queue, иначе app.job_queue is None:
    # без сводки бот должен работать, а не падать на старте.
    job_queue = app.job_queue
    if job_queue is None:
        logger.warning(
            "JobQueue недоступен — еженедельная сводка отключена. "
            'Установи: pip install "python-telegram-bot[job-queue]"'
        )
    else:
        job_queue.run_daily(
            weekly_summary,
            time=dt_time(hour=19, minute=0, tzinfo=LOCAL_TZ),
            name="weekly_summary",
        )
